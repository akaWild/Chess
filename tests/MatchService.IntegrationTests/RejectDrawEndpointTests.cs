...
namespace MatchService.IntegrationTests
{
    ...
    public class RejectDrawEndpointTests : EndpointTestsBase
    {
        private readonly string? _tokenWithoutUser;
        private readonly string? _tolianToken;
        private readonly string? _kolianToken;

        private DrawRejectedDto? _drawRejectedDto;
        private MatchFinishedDto? _matchFinishedDto;

        public RejectDrawEndpointTests(CustomWebAppFactory factory) : base(factory)
        {
        ...
        [Fact]
        public async Task RejectDraw_WithoutToken()
        {
            //Arrange
            ...
            Exception? exception = await Record.ExceptionAsync(async () => await hubConnection.InvokeAsync("RejectDraw", matchId));
        ...
        [Fact]
        public async Task RejectDraw_WithoutUser()
        {
            //Arrange
            var matchId = Guid.Parse("7139D633-66F9-439F-8198-E5E18E9F6848");
        ...
        [Fact]
        public async Task RejectDraw_WithUnknownMatchId()
        {
            //Arrange
        ...
        [Fact]
        public async Task RejectDraw_WithInvalidUser()
        {
            //Arrange
        ...
        [Fact]
        public async Task RejectDraw_WithInvalidMatchStatus()
        {
            //Arrange
        ...
        [Fact]
        public async Task RejectDraw_WithoutPrevDrawRequest()
        {
            //Arrange
        ...
        [Fact]
        public async Task RejectDraw_WithRejectedDrawByIdleSideAsWhite()
        {
            //Arrange
        ...
        [Fact]
        public async Task RejectDraw_WithRejectedDrawByIdleSideAsBlack()
        {
            //Arrange
        ...
        [Fact]
        public async Task RejectDraw_WithValidInputWithoutTimeout()
        {
            //Arrange
            var matchId = Guid.Parse("BD0C1A6B-6BD0-4457-B4BA-74BD3ABD45C3");
            var hubConnection = HubConnectionHelper.GetHubConnection(HttpMessageHandler, token: _tolianToken, matchId: matchId.ToString());
            ...
            Assert.NotNull(_drawRejectedDto);
            Assert.Equal(matchId, _drawRejectedDto.MatchId);
            Assert.NotNull((await Harness.Published.SelectAsync<DrawRejected>().ToListAsync()).FirstOrDefault(v => v.Context.Message.MatchId == matchId));
        }

        [Fact]
        public async Task RejectDraw_WithValidInputWithTimeout()
        {
            //Arrange
        ...
        protected override void SetConnectionHandlers(HubConnection hubConnection)
        {
            ...
            hubConnection.On<DrawRejectedDto>("DrawRejected", (dto) =>
            {
                _drawRejectedDto = dto;
